...
#import "DLPointParticleEffect.h"
#import "DLVertexAttribArrayBuffer.h"
//用于定义粒子属性的类型
typedef struct
{
...
}DLParticleAttributes;

enum
{
    DLMVPMatrix,//MVP矩阵
    DLSamplers2D,//Samplers2D纹理
    DLElapsedSeconds,//耗时
    DLGravity,//重力
    DLNumUniforms//Uniforms个数
};

typedef enum
{
    DLParticleEmissionPosition = 0,//粒子发射位置
    DLParticleEmissionVelocity,//粒子发射速度
    DLParticleEmissionForce,//粒子发射重力
    DLParticleSize,//粒子发射大小
    DLParticleEmissionTimeAndLife,//粒子发射时间和寿命
    
}DLParticleAttrib;

@interface DLPointParticleEffect()
{
    GLfloat elapsedSeconds;//耗时
    GLuint program;//程序
    GLint uniforms[DLNumUniforms];//Uniforms数组
}

//顶点数组缓冲区
@property(nonatomic, strong, readwrite)DLVertexAttribArrayBuffer *particleAttributeBuffer;
...
@implementation DLPointParticleEffect
...
//获取粒子的属性值
- (DLParticleAttributes)particleAtIndex:(NSUInteger)index{
    //bytes:指向接收者内容的指针
    //获取粒子属性结构体内容
    const DLParticleAttributes *particlesPtr = (const DLParticleAttributes *)[self.particleAttributesData bytes];
    //获取属性结构体中的某一个指标
    return particlesPtr[index];
}

//设置粒子的属性
-(void)setParticle:(DLParticleAttributes)aParticle
           atIndex:(NSUInteger)index
{
    //mutableBytes:指向可变数据对象所包含数据的指针
    //获取粒子属性结构体内容
    DLParticleAttributes *particlesPtr = (DLParticleAttributes *)[self.particleAttributesData mutableBytes];
...
{
    //创建新的粒子
    DLParticleAttributes newParticle;
    ...
    {
        //获取当前旧的粒子
        DLParticleAttributes oldParticle = [self particleAtIndex:i];
        
        //如果旧的粒子发射时长小于耗时
        //emissionTimeAndLife.y = elspsedSeconds + aspan
...
-(NSUInteger)numberOfParticles{
    static long last;
    //总数据/粒子结构体大小
    long ret = [self.particleAttributesData length] / sizeof(DLParticleAttributes);
...
-(void)prepareToDraw{
    ...
    if(program != 0){
         ...
         */
        glUniformMatrix4fv(uniforms[DLMVPMatrix], 1, 0, modelViewProjectionMatrix.m);
         ...
         */
        glUniform1i(uniforms[DLSamplers2D], 0);
         ...
         */
        glUniform3fv(uniforms[DLGravity], 1, self.gravity.v);
        
          //耗时
        glUniform1fv(uniforms[DLElapsedSeconds], 1, &elapsedSeconds);
        ...
        if(self.particleDataWasUpdated)
        {
            ...
            { // 顶点属性没有送到GPU
                 ...
                 */
                //数据大小
                GLsizeiptr size = sizeof(DLParticleAttributes);
                //个数
                int count = (int)[self.particleAttributesData length] / sizeof(DLParticleAttributes);
                
                self.particleAttributeBuffer = [[DLVertexAttribArrayBuffer alloc] initWithAttribStride:size numberOfVertices:count bytes:[self.particleAttributesData bytes] usage:GL_DYNAMIC_DRAW];
        ...
        //准备顶点数据
        [self.particleAttributeBuffer prepareToDrawWithAttrib:DLParticleEmissionPosition numberOfCoordinates:3 attribOffset:offsetof(DLParticleAttributes, emissionPosition) shouldEnable:YES];
        
        //准备粒子发射速度数据
        [self.particleAttributeBuffer prepareToDrawWithAttrib:DLParticleEmissionVelocity numberOfCoordinates:3 attribOffset:offsetof(DLParticleAttributes, emissionVelocity) shouldEnable:YES];
        
        //准备重力数据
        [self.particleAttributeBuffer prepareToDrawWithAttrib:DLParticleEmissionForce numberOfCoordinates:3 attribOffset:offsetof(DLParticleAttributes, emissionForce) shouldEnable:YES];
        
        //准备粒子size数据
        [self.particleAttributeBuffer prepareToDrawWithAttrib:DLParticleSize numberOfCoordinates:2 attribOffset:offsetof(DLParticleAttributes, size) shouldEnable:YES];
        
        //准备粒子的持续时间和渐隐时间数据
        [self.particleAttributeBuffer prepareToDrawWithAttrib:DLParticleEmissionTimeAndLife numberOfCoordinates:2 attribOffset:offsetof(DLParticleAttributes, emissionTimeAndLife) shouldEnable:YES];
...
-(BOOL)loadShaders{
    ...
    //创建并编译vertex shader
    vertShaderPathname = [[NSBundle mainBundle] pathForResource:@"DLPointParticleShader" ofType:@"vsh"];
    ...
    fragShaderPathname = [[NSBundle mainBundle] pathForResource:@"DLPointParticleShader" ofType:@"fsh"];
    ...
    //位置
    glBindAttribLocation(program, DLParticleEmissionPosition, "a_emissionPosition");
    
    //速度
    glBindAttribLocation(program, DLParticleEmissionVelocity, "a_emissionVelocity");
    
    //重力
    glBindAttribLocation(program, DLParticleEmissionForce, "a_emissionForce");
    
    //持续时间，渐隐时间
    glBindAttribLocation(program, DLParticleEmissionTimeAndLife, "a_emissionAndDeathTimes");
    ...
    // 获取uniform变量的位置.
    //MVP变换矩阵
    uniforms[DLMVPMatrix] = glGetUniformLocation(program, "u_mvpMatrix");
    //纹理
    uniforms[DLSamplers2D] = glGetUniformLocation(program, "u_samplers2D");
    //重力
    uniforms[DLGravity] = glGetUniformLocation(program, "u_gravity");
    //持续时间，渐隐时间
    uniforms[DLElapsedSeconds] = glGetUniformLocation(program, "u_elapsedSeconds");
...
//默认重力
const GLKVector3 DLDefaultGravity = {0.0f, -9.80665f, 0.0f};
