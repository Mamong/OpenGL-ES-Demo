...
attribute vec2 a_size;                      //大小和Fade持续时间 size = GLKVector2Make(aSize, aDuration);
attribute vec2 a_emissionAndDeathTimes;     //发射时间 和 消失时间

//UNIFORMS
uniform highp mat4          u_mvpMatrix;     //变化矩阵
uniform sampler2D           u_sampler2D[1];  //纹理
...
void main()
{
    ...
    // 如果这个值小于0，则取0
    v_particleOpacity = max(0.0, min(1.0,(a_emissionAndDeathTimes.y - u_elapsedSeconds) / max (a_size.y, 0.0001) ));
}
