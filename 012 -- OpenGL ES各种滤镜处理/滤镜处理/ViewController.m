...
//  滤镜处理
//
//  Created by denglei on 2019/6/27.
...
- (void)setupFilterBar {
    ...
    NSArray *dataSource = @[@"无",@"灰度",@"颠倒",@"旋涡",@"马赛克",@"六边形马",@"三角形马",@"缩放",@"出窍",@"抖动",@"闪白",@"撕裂"];
...
- (void)filterInit {
    ...
    //设置图层frame
    layer.frame = CGRectMake(0, 100, self.view.frame.size.width, 500);
    //设置图层的scale
    ...
    //6.获取处理的图片路径
    NSString *imagePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"test2.jpg"];
    //读取图片
...
- (void)setupVortexShaderProgram {
    //设置着色器程序
    [self setupShaderProgramWithName:@"Vortex"];
}
